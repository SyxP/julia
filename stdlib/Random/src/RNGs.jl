...
# by default, assume RNGs produces natively UInt64
...
function random_seed()
    try
        ...
        # almost surely getting distinct seeds, while having them printed reasonably tersely
...
"""
    hash_seed(seed, ctx::SHA_CTX) -> AbstractVector{UInt8}
...
Moreover, if `n == m`, then `hash_seed(n, cn) == hash_seed(m, cm)`.
"""
hash_seed
...
"""
    SeedHasher(seed=nothing)

Create a `Random.SeedHasher` RNG object, which generates random bytes with the help
of a cryptographic hash function (SHA2).

Given two distinct seeds `s1` and `s2`, the random streams generated by
`SeedHasher(s1)` and `SeedHasher(s2)` should be different.

This RNG is used by default in `seed!(::AbstractRNG, seed::Any)`, such that
...
@noinline function rehash!(rng::SeedHasher)
    ...
    # if this happens, it means we found a collision (practically impossible), but if we
    # changed slightly the involved algorithms, this could happen without a SHA2 collision,
    # so with an unknown probably, so stay on the safe side
...
seed!(r::MersenneTwister, seeder::AbstractRNG) = initstate!(r, rand(seeder, UInt32, 24))
...
seed!(r::MersenneTwister, seed) =
    initstate!(r, rand(SeedHasher(@something seed random_seed()), UInt32, 24), seed)
